...
const exportAsJson = () => {
  ...
  link.href = url
  link.download = `africa-prompt-export-${Date.now()}.json`
...
const exportAsMarkdown = () => {
  ...
  const link = document.createElement('a')
  link.href = url
  link.download = `africa-prompt-export-${Date.now()}.md`
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
  URL.revokeObjectURL(url)
...
<template>
  <div class="flex items-center space-x-2">
    ...
    >
      ...
      <span v-else class="text-green-300">{{ t('jsonDownloaded') }}</span>
    </button>
    ...
    >
      ...
      <span v-else class="text-green-300">{{ t('markdownDownloaded') }}</span>
    </button>
  </div>
</template>
